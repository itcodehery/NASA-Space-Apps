...
import { fetchStateEmissions } from "../api";
import "./map.css";
...
export default function StateMap({ filters = ['CO2', 'CH4', 'CO'], year = 2024 }) {
  ...
  const getStateColor = (state) => {
    ...
    const activeGasValues = activeGases.map(gas => ({
      gas,
      value: state[gas] || 0
    })).filter(g => g.value > 0);
  ...
  const getStateOpacity = (state) => {
    ...
    const maxEmissions = Math.max(...dummyStateData.map(s => s.totalEmissions));
      ...
      <div className="state-map-container">
        ...
        {hoveredState && (
          <div className="absolute top-4 right-4 bg-gray-900/95 border border-gray-700 p-4 z-10 rounded-none">
            <h3 className="text-white font-semibold mb-2">{hoveredState.stateName}</h3>
